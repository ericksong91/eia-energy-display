...
function SearchBar({ onUpdateGraphs }) {
  const { upArrow, downArrow, magnifyingGlass, states } = useContext(IconContext);
  ...
  return (
    <div className='search-bar-combobox w-full mx-auto'>
      ...
      <div className="relative">
        ...
        <ul className={`select-options hide-scrollbar w-full max-h-64 overflow-y-scroll flex-col ${isOpen ? 'bg-white rounded-lg shadow-lg p-3 z-50 dark:bg-dark-background' : ''}`} {...getMenuProps()}>
          {isOpen &&
            inputItems.map((item, index) => (
              <li key={`${item}${index}`} {...getItemProps({ item, index, })}
                className={`${item}-option ${selectedItem === item ? 'font-bold' : 'font-normal'} ${highlightedIndex === index ? 'bg-blue-100' : ''} cursor-pointer`}>
                {item}
              </li>
            ))}
        </ul>
